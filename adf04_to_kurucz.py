...
parser.add_argument('-r','--reject_bad_a_values',  help='Reject bad A values? i.e 1e-30 in adf04 file (off by default)',type=int)
...
if not args.file:
    ...
    if args.reject_bad_a_values:
        ...
        if user_input_a_values == 0:
            reject_bad_a_values = False
        elif user_input_a_values == 1:
...
def main():
    ...
    output.write_out_kurucz_fortran_format(lower_levels,upper_levels,jvalues,wavelengths,a_values_float,log_gf,energy_levels_cm_minus_one,elementcode,csfs_strings,level_truncate=num_requested_lines,reject_bad_a_values=reject_bad_a_values)
